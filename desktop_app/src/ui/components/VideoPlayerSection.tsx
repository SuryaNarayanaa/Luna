...
import type { PlayerState, Speaker } from '../types';
...
interface VideoPlayerSectionProps {
  ...
  onSpeakerFocus: (speakerId?: string) => void;
...
export function VideoPlayerSection({
  ...
  return (
    <div className="bg-zinc-900 border-b border-zinc-800 sticky top-0 z-10">      {/* Video Player Container - Larger Size for Video View */}
<div className="flex justify-center px-4 pt-4 pb-3">
        <div className="relative w-full max-w-4xl aspect-[16/9] bg-black rounded-lg overflow-hidden shadow-2xl border border-zinc-700">
          {/* Mock Video Player */}
          <div className="w-full h-full bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center">            <div className="text-center">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => onSpeakerFocus(undefined)}
                className={`px-2 py-1 text-xs rounded-lg transition-colors ${
                  !playerState.selectedSpeaker
              ...
              {speakers.map((speaker) => (
                <button
                  key={speaker.id}
                  onClick={() => onSpeakerFocus(speaker.id)}
                  className={`px-2 py-1 text-xs rounded-lg transition-colors flex items-center space-x-1 ${
                    playerState.selectedSpeaker === speaker.id
                      ? 'text-white'
                      : 'text-zinc-400 hover:text-white hover:bg-zinc-800'
                  }`}
                  style={{
                    backgroundColor: playerState.selectedSpeaker === speaker.id 
                      ? speaker.color 
                      : undefined
                  }}
                >
                  <User className="w-3 h-3" />
                  <span>{speaker.name.split(' ')[0]}</span>
                </button>
              ))}
            </div>
            ...
            {/* Sync Lock */}            <div className="flex items-center space-x-1">
              <input
                type="checkbox"
                id="sync-lock"
                checked={playerState.transcriptSync}
                className="rounded border-zinc-600"
                readOnly
              />
              <label htmlFor="sync-lock" className="text-xs text-zinc-400">
                Sync
              </label>
            </div>
          </div>
